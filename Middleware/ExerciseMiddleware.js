export const ExerciseMiddleware = async (req, res, next) => {
  ...
  if (!data.targetBodyParts) {
    ...
    try {
      const parts =
        typeof data.targetBodyParts === "string"
...
/** Same as create validation, but image is optional (keep existing if omitted). */
export const ExerciseUpdateMiddleware = async (req, res, next) => {
  const data = req.body;
  const errors = {};
